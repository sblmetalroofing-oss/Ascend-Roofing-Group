 ...
 */
export function validateUpload(file, { label = "file", maxBytes = 3 * 1024 * 1024 } = {}) {
...
// Raw (unescaped) trimmed text for storage. Escape with sanitize() only when
// rendering into HTML — escaping before storage inflates values past the
// VARCHAR widths and double-escapes on later render.
...
// Returns the first key whose value is longer than its column limit, or null.
export function findTooLong(values, limits) {
