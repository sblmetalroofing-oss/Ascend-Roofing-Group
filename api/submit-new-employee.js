    ...
    try {
            ...
            try {
                const result = await sql`
                    ...
                    VALUES (
                        ...
                        ${safe.tfn}, ${safe.taxFreeThreshold}, ${safe.australianResident}, ${safe.hasHelpDebt},
                        ${safe.superFundName}, ${safe.superMemberNumber}, ${safe.superUsi},
                        ${safe.bsb}, ${safe.accountNumber}, ${safe.accountName}
                    )
                    ON CONFLICT (email)
                    DO UPDATE SET
                ...
                for (const { key, type } of fileTypes) {
                    if (files[key]) {
                        await sql`
                            ...
                            VALUES (${employeeId}, ${type}, ${sanitize(files[key].name)})
                        `;
                    }
                }
        ...
        const { data, error } = await resend.emails.send({
            ...
            html: `
                ...
                <h3>Tax Details</h3>
                <table style="border-collapse:collapse; width:100%; max-width:600px; margin-bottom:24px;">
                    <tr><td style="padding:8px; border:1px solid #ddd; font-weight:bold;">TFN</td><td style="padding:8px; border:1px solid #ddd;">${safe.tfn}</td></tr>
                ...
                <h3>Bank Account</h3>
                <table style="border-collapse:collapse; width:100%; max-width:600px; margin-bottom:24px;">
                    <tr><td style="padding:8px; border:1px solid #ddd; font-weight:bold;">BSB</td><td style="padding:8px; border:1px solid #ddd;">${safe.bsb}</td></tr>
                    <tr><td style="padding:8px; border:1px solid #ddd; font-weight:bold;">Account Number</td><td style="padding:8px; border:1px solid #ddd;">${safe.accountNumber}</td></tr>
                ...
                <p style="color:#666; font-size:12px; margin-top:24px;">
                    <em>Photo ID, White Card, and any additional certifications are attached to this email. Treat the
                    TFN, banking, and ID details as confidential.</em>
                </p>
            `
        });
